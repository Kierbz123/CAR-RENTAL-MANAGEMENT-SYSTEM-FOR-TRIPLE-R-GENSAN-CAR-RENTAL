    ...
    async function refresh() {
        try {
            ...
            rows.innerHTML = items.length ? items.map((item) => `<tr>
                ...
                <td>${escapeHtml(item.provider)}</td><td>${escapeHtml(item.last_error || '—')}</td></tr>`).join('') : '<tr><td colspan="10">No notifications have been queued.</td></tr>';
            ...
            if (!rows.children.length) rows.innerHTML = '<tr><td colspan="10">History is temporarily unavailable.</td></tr>';
        }
    }
