...
| `app/Repositories/NotificationRepository.php` | Backend | Modified | Persist encrypted bodies, redact encrypted bodies after terminal delivery/failure, and detect due encrypted jobs before worker claims. |
...
The token table stores only the hash. To deliver a bearer link through Feature E's persistent queue without writing its raw SMS body in plaintext, the queue stores `smsenc:v1:` plus AES-256-GCM ciphertext in `notifications.rendered_message`. `SMS_CIPHER_KEY` is a dedicated base64-encoded 32-byte key and must not reuse `APP_KEY` or a session key. `SMS_CIPHER_KEY_PREVIOUS` is optional during key rotation. The worker decrypts the body only immediately before provider send. It deletes encrypted body content after a terminal send, suppression, or final provider failure; retryable rows retain ciphertext until another attempt is due. If an encrypted job is due but no valid current/previous key is configured, the worker exits before claiming it. A decryption or worker configuration failure preserves ciphertext for key recovery and marks the row failed for manual reconciliation.

The authenticated staff API returns a `message_preview`, never `rendered_message`. Ordinary content can be read by staff; `magic_link.*` is always `[Magic-link content masked]`. Ciphertext is never rendered. If a non-magic encrypted template is later added, the server decrypts its preview; failure returns `[Encrypted message unavailable]`. To rotate keys, pause the worker, install the new current key and old key as previous, then resume. Retain the old key until no old-key encrypted rows remain in `notifications`, including failed rows retained after decryption or worker configuration problems; drain or securely remove them before a subsequent rotation because the app supports one previous key.
