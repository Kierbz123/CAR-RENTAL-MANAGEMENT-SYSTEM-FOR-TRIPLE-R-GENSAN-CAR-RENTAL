...
`RESERVATION_HOLD_MINUTES` (default 60) is separate from no-show grace. Agreement status `reserved` means a temporary unconfirmed hold; vehicle `current_status='reserved'` means a confirmed commitment exists. Under the current behavior, any confirmed future agreement reserves the fleet status immediately, even if pickup is weeks away; the overlap-based booking selector can still accept other non-overlapping dates. Vehicle status mapping is: creating a `reserved` hold does not change fleet status; `reserved → confirmed` changes `available → reserved` (or leaves an already-reserved vehicle reserved); `confirmed → active` changes `reserved → rented`; `active → returned` reconciles to `rented` if another agreement is active, otherwise `reserved` if another confirmed agreement ending today or later exists, otherwise `available`. Cancelling or marking a confirmed agreement no-show uses that same reconciliation; cancelling/no-show on an unconfirmed hold and expiring a reserved hold do not change vehicle status. Expired holds are marked cancelled by `bin/rentals-expire.php` and booking links are invalidated. The display meaning of far-future `reserved` is pending the owner's decision; the broad behavior is documented as current implementation, not a settled business policy.
...
- Near UTC/Manila midnight, verify a confirmed agreement whose `end_date` is today in Manila is treated as current/future regardless of the UTC server date. Also create a pickup scheduled just after Manila midnight (the prior UTC date), and confirm its local date/time is converted and displayed as intended; separately check distant-booking status against the owner's decision on `reserved` semantics.
