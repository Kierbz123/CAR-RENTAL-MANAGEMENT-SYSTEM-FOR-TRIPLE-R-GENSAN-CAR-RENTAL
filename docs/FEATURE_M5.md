...
`RESERVATION_HOLD_MINUTES` (default 60) is separate from no-show grace. Agreement status `reserved` means a temporary unconfirmed hold; vehicle `current_status='reserved'` means a confirmed commitment exists. The chosen live fleet-list policy is immediate: any confirmed future agreement reserves the fleet status, even if pickup is weeks away. This is a conservative display choice, not a booking-safety mechanism; the transaction-level overlap check independently permits other non-overlapping dates. Vehicle status mapping is: creating a `reserved` hold does not change fleet status; `reserved → confirmed` changes `available → reserved` (or leaves an already-reserved vehicle reserved); `confirmed → active` changes `reserved → rented`; `active → returned` reconciles to `rented` if another agreement is active, otherwise `reserved` if another confirmed agreement ending today or later exists, otherwise `available`. Cancelling or marking a confirmed agreement no-show uses that same reconciliation; cancelling/no-show on an unconfirmed hold and expiring a reserved hold do not change vehicle status. Expired holds are marked cancelled by `bin/rentals-expire.php` and booking links are invalidated.

Return/cancel/no-show release uses the single `RentalService::reconcileVehicleStatus()` method, which checks active agreements before confirmed future agreements and skips a no-op status transition. This prevents a later reservation's cancellation from changing a vehicle that remains rented on an earlier active agreement. The local runtime checklist includes the two-agreement case (one active, one later confirmed; cancel the later one and verify the vehicle remains `rented`).
...
The canonical multi-row lock order is **vehicle → customer → driver → agreement**. Creation locks vehicle then customer before overlap checks/insertion. Lifecycle operations read an unlocked agreement snapshot only to discover immutable `vehicle_id` and `customer_id`, then lock vehicle → customer → agreement; M6 inserts the driver lock before agreement, preserving the same complete sequence. Migration `008_rental_agreement_identity_immutable.sql` adds a database trigger rejecting changes to either identity column, in addition to the absence of any application reassignment path. Deposit and charge operations lock only their agreement row. M7 and later multi-row work must use the same sequence. Vehicle state transitions and mileage recording use M2's transaction-aware service methods so agreement, fleet status, mileage, and history commit or roll back together.
...
Run `php bin/migrate.php` to apply migration 007 after 001–006 and migration 008's identity guard. Add the three every-minute jobs shown in the README: rental hold expiry, pickup/return reminders, and STOP import. Configure `RESERVATION_HOLD_MINUTES`, `NO_SHOW_GRACE_MINUTES`, and `NON_TRANSACTIONAL_SMS_ENABLED` in `.env` as needed.
...
- A far-future confirmed agreement immediately makes the fleet-list status `reserved`; a new non-overlapping date range for that vehicle remains bookable. Period-based M10 availability must use agreement date ranges and/or historical `vehicle_status_logs`, never the current-status snapshot alone.
...
- With one active agreement and a later non-overlapping confirmed agreement on the same vehicle, cancelling the later agreement leaves fleet status `rented`; returning the active agreement while another confirmed future agreement remains sets it to `reserved`.
- Near UTC/Manila midnight, verify a confirmed agreement whose `end_date` is today in Manila is treated as current/future regardless of the UTC server date. Also create a pickup scheduled just after Manila midnight (the prior UTC date), and confirm its local date/time is converted and displayed as intended.
- Migration 008 rejects direct SQL UPDATE attempts changing `rental_agreements.vehicle_id` or `customer_id`, while ordinary status/deposit/timestamp updates succeed.
