...
3. A later feature calls `NotificationService::enqueue` → the service normalizes the phone, encrypts the body, applies idempotency/budgets, and records non-transactional attempts as `suppressed_by_policy` until affirmative opt-in exists → caller receives its notification ID.
...
6. Provider inbound STOP callback → HMAC validation → normalize the sender and classify the message → insert the full raw body and normalized event into `inbound_sms_events` using its unique provider message ID → HTTP 200 acknowledgement. M5 imports STOPs into append-only `rules_acceptances`; transactional notifications remain permitted after STOP, while non-transactional attempts are audibly policy-suppressed.
